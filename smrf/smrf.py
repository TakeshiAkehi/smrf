...
def smrf(
    ...
    col_centers = np.arange(0.5, Zpro.shape[1] + 0.5)
    row_centers = np.arange(0.5, Zpro.shape[0] + 0.5)
...
def progressive_filter(Z, windows, cellsize=1, slope_threshold=0.15, return_when_dropped=False):
    last_surface = Z.copy()
    ...
    for i, window in enumerate(windows):
        LOGGER.info("progressive filter : %d/%d (window_size=%d)" % (i, len(windows), window))
...
def inpaint_nans_by_fda(A, fast=True):
    ...
    i = np.hstack((np.tile(index[1:-1, :].ravel(), 3), np.tile(index[:, 1:-1].ravel(), 3)))
    j = np.hstack(
        (
            index[0:-2, :].ravel(),
            index[2:, :].ravel(),
    ...
    data = np.hstack(
        (
            np.ones(2 * n * (m - 2), dtype=np.int64),
...
def create_dem(x, y, z, cellsize=1, bin_type="max", inpaint=False, edges=None, use_binned_statistic=False):
    ...
    if edges is None:
        xedges = np.arange(
            floor2(np.min(x), cellsize) - 0.5 * cellsize, ceil2(np.max(x), cellsize) + 1.5 * cellsize, cellsize
        )
        yedges = np.arange(
            ceil2(np.max(y), cellsize) + 0.5 * cellsize, floor2(np.min(y), cellsize) - 1.5 * cellsize, -cellsize
        )
    else:
        xedges = edges[0]
        yedges = edges[1]
...
def read_las(filename):
    ...
    if header["point_data_format_id"] >= 6:
        print(
            "Point Data Formats 6-10 have recently been added to this reader.  Please check results carefully and report any suspected errors."
        )
    ...
    if header["point_data_format_id"] == 0:
        ...
        dt = np.dtype(
            [
                ...
                ("point_source_id", "u2"),
                ("gpstime", "f8"),
                ("wave_packet_descriptor_index", "u1"),
                ("byte_offset", "u8"),
                ("wave_packet_size", "u4"),
    ...
    if header["point_data_format_id"] < 6:
        data["return_number"] = (
            4 * get_bit(data["return_byte"], 2).astype(np.uint8)
            + 2 * get_bit(data["return_byte"], 1).astype(np.uint8)
            + get_bit(data["return_byte"], 0).astype(np.uint8)
        )
        data["return_max"] = (
            4 * get_bit(data["return_byte"], 5).astype(np.uint8)
            + 2 * get_bit(data["return_byte"], 4).astype(np.uint8)
            + get_bit(data["return_byte"], 3).astype(np.uint8)
        )
        ...
        data["return_number"] = (
            8 * get_bit(data["return_byte"], 3).astype(np.uint8)
            + 4 * get_bit(data["return_byte"], 2).astype(np.uint8)
            + 2 * get_bit(data["return_byte"], 1).astype(np.uint8)
            + get_bit(data["return_byte"], 0).astype(np.uint8)
        )
        data["return_max"] = (
            8 * get_bit(data["return_byte"], 7).astype(np.uint8)
            + 4 * get_bit(data["return_byte"], 6).astype(np.uint8)
            + 2 * get_bit(data["return_byte"], 5).astype(np.uint8)
            + get_bit(data["return_byte"], 4).astype(np.uint8)
        )
        # data['scan_direction'] = get_bit(data['return_byte'],6)
        # data['edge_of_flight_line'] = get_bit(data['return_byte'],7)
        del data["return_byte"]
    if header["point_data_format_id"] >= 6:
        ...
        data["scanner_channel"] = 2 * get_bit(data["mixed_byte"], 5).astype(np.uint8) + 1 * get_bit(
            data["mixed_byte"], 4
        ).astype(np.uint8)
